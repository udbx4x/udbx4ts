...
- 为后续扩展 3D 数据集、CAD 数据集和性能优化预留架构空间
...
具体数据集类型如 `PointDataset`、`LineDataset`、`RegionDataset` 只承担以下职责：
...
- 如果一开始就同时做 CAD、3D、空间索引、复杂 UI 示例，开发周期会显著拉长
...
按照本方案实施，可以更稳妥地完成 Web 与 Electron 双平台目标，并为后续扩展 3D、CAD 和性能优化打下可维护的基础。
