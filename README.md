...
| Text 数据集 | ✅ 最小 GeoText 读/写 | ✅ 最小 GeoText 读/写 |
...
已完整实现的 Point、Line、Region、PointZ、LineZ、RegionZ、Tabular、Text 最小 GeoText 和 CAD 最小 GeoHeader 数据集支持以下操作。
...
`udbx4ts` 的公开 API 遵循 `udbx4spec/docs/08-api-stable-surface.md`。TypeScript 侧统一使用异步接口，浏览器、Node.js 和 Electron 运行时不得改变核心语义。

| 语义 | TypeScript API | 稳定行为 |
|---|---|---|
| 打开/创建数据源 | `UdbxDataSource.open` / `UdbxDataSource.create` / 运行时工厂 | 打开或创建 UDBX；运行时差异不得泄漏到核心语义 |
| 列出数据集 | `listDatasets()` | 返回 `DatasetInfo` 列表 |
| 按名称获取数据集 | `getDataset(name)` | 不存在时 reject `UdbxNotFoundError` |
| 按 ID 获取对象 | `getById(id)` | 不存在时 reject `UdbxNotFoundError`，不 resolve `null` |
| 列表读取 | `list(options?)` / `iterate(options?)` | 默认按 `SmID` 升序返回 |
| 计数 | `count()` | 读取物理表真实行数，不以 `SmRegister.SmObjectCount` 缓存为准 |
| 写入 | `insert(...)` / `insertMany(...)` | 写入对象并同步对象数 |
| 更新 | `update(id, changes)` | 目标不存在时 reject `UdbxNotFoundError`；未知字段返回 not found 或约束类错误 |
| 删除 | `delete(id)` | 目标不存在时 reject `UdbxNotFoundError`；成功后同步对象数 |
